...
const CertificateCarousel = () => {
  ...
  return <div className="relative px-12">
      <Carousel opts={{
      align: "start",
      loop: true
    }} className="w-full">
        <CarouselContent className="-ml-4">
          {certificates.map(certificate => <CarouselItem key={certificate.id} className="pl-4 md:basis-1/2 lg:basis-1/3">
              <div className="group cursor-pointer transform transition-all duration-300 hover:scale-105" onClick={() => handleCertificateClick(certificate)}>
                <div className="relative overflow-hidden rounded-lg shadow-lg bg-gray-800 border border-gray-700 hover:border-gray-600 transition-colors">
                  <div className="aspect-[4/3] overflow-hidden">
                    <img src={certificate.image_url} alt={certificate.title} className="w-full h-full object-cover transition-transform duration-300 group-hover:scale-110" />
                  </div>
                  <div className="p-6">
                    <h3 className="text-xl font-medium mb-2 text-white group-hover:text-blue-400 transition-colors">
                      {certificate.title}
                    </h3>
                    {certificate.issued_by && <p className="text-sm text-gray-400 mb-2">
                        Issued by {certificate.issued_by}
                      </p>}
                    {certificate.issued_date && <p className="text-sm text-gray-500">
        ...
        <CarouselPrevious className="border-gray-600 text-gray-950 bg-zinc-50" />
