...
import {
  Code, Figma, FileType, Wind, Braces, LayoutTemplate,
  FileCode, Zap
} from "lucide-react";
                ...
                {skillsData.map((skill, index) => (
                  <HoverCard key={index} openDelay={100} closeDelay={100}>
                    <HoverCardTrigger asChild>
                      <Card 
                        className={`cursor-pointer hover:shadow-md transition-all duration-300 group h-24 sm:h-28 flex items-center justify-center ${hoveredSkill === skill.name ? 'ring-2 ring-black' : ''}`}
                    ...
                    <HoverCardContent 
                      side="bottom" 
                      align="center" 
                      className="w-72 p-4 shadow-lg bg-white border-0"
                      sideOffset={10}
                    >
                      <motion.div
                        initial="initial"
