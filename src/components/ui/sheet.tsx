...
// The bulge effect variants for our motion component
const bulgeAnimationVariants = {
  initial: { 
    x: "100%" 
  },
  animate: (side: "left" | "right" | "top" | "bottom" = "right") => {
    if (side === "left") {
      return {
        x: 0,
        transition: {
...
const SheetContent = React.forwardRef<React.ElementRef<typeof SheetPrimitive.Content>, SheetContentProps>(({
  ...
  <SheetPortal>
    <SheetOverlay />
    <SheetPrimitive.Content asChild ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
      <motion.div
        initial="initial"
        animate="animate"
        exit="exit"
        variants={bulgeAnimationVariants}
        custom={side}
      >
        {children}
